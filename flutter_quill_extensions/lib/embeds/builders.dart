    ...
    if (style != null) {
      ...
      if (attrs.isNotEmpty) {
        final width = double.tryParse(
          (base.isMobile() ? attrs[Attribute.mobileWidth] : attrs[Attribute.width.key]) ?? '',
        );
        final height = double.tryParse(
          (base.isMobile() ? attrs[Attribute.mobileHeight] : attrs[Attribute.height.key]) ?? '',
        );
        final alignment = base.getAlignment(base.isMobile() ? attrs[Attribute.mobileAlignment] : attrs[Attribute.alignment]);
        final margin = (base.isMobile() ? double.tryParse(Attribute.mobileMargin) : double.tryParse(Attribute.margin)) ?? 0.0;

        assert(
          width != null && height != null,
          base.isMobile() ? 'mobileWidth and mobileHeight must be specified' : 'width and height must be specified',
        );
              ...
              builder: (context) {
                ...
                final removeOption = _SimpleDialogItem(
                  ...
                  onPressed: () async {
                    ...
                    if (await shouldRemoveImageCallback?.call(imageFile) == false) {
                      return;
                    }
