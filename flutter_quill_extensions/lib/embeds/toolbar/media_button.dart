...
class MediaSourceSelectorDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shape = dialogTheme?.shape ?? DialogTheme.of(context).shape ?? RoundedRectangleBorder(borderRadius: BorderRadius.circular(4));
...
class TextButtonWithIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shape = buttonStyle?.shape?.resolve({}) ?? const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(4)));
    return Material(
      shape: shape,
...
Future<QuillFile?> _defaultMediaPicker(QuillMediaType mediaType) async {
  final pickedFile =
      mediaType.isImage ? await ImagePicker().pickImage(source: ImageSource.gallery) : await ImagePicker().pickVideo(source: ImageSource.gallery);
